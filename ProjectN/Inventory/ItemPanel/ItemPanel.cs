...
public class ItemPanel : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, IDragHandler, IDropHandler, IEndDragHandler
{
	...
	[HideInInspector] public StorageView inventory;
	...
	{
		int slotAvailable = destination.item.GetMaxStack() - destination.stacks;
		if (slotAvailable == 0) return;

		if (amount > slotAvailable)
		{
			source.stacks -= slotAvailable;
			destination.stacks = destination.item.GetMaxStack();
	...
	private void OnClick()
	{
		if (inventory != null)
		{
			...
			else
			{
				...
				else if (itemSlot.item.Name != _mouse.itemSlot.item.Name)
				{
					SwapItem(itemSlot, _mouse.itemSlot);
					inventory.Refresh();
				}

				else if (itemSlot.stacks < itemSlot.item.GetMaxStack())
				{
